...
(async () => {
  ...
  assert((await page.locator('body').innerText()).includes('3 / 9'), 'keyboard right did not advance');
  await page.getByRole('button', { name: /검수/ }).first().click();
  assert((await page.locator('body').innerText()).includes('4 / 9'), 'direct slide jump did not update progress');
  await page.getByPlaceholder('예: 근거 발언 확인 완료').fill('스모크 테스트 확인');
  await page.locator('.admin-slide-card:not([aria-hidden="true"])').getByRole('button', { name: '승인' }).first().click();
  await page.getByRole('button', { name: /로그/ }).first().click();
  const auditBody = await page.locator('body').innerText();
  assert(auditBody.includes('스모크 테스트 확인'));
  ...
  assert.equal(persistentAdminNav.position, 'fixed', `mobile admin global nav should be fixed, got ${JSON.stringify(persistentAdminNav)}`);
  assert(persistentAdminNav.visible, `mobile admin global nav disappeared after deep scroll: ${JSON.stringify(persistentAdminNav)}`);
  assert(persistentAdminNav.top >= 0 && persistentAdminNav.bottom <= persistentAdminNav.viewportHeight, `mobile admin global nav should remain in viewport: ${JSON.stringify(persistentAdminNav)}`);
